...
class ISAC_SatEnv(gym.Env):
    ...
    def _log_to_wandb(self, reward, info):
        """记录数据到WandB"""
        log_data = {
            "reward": reward,
        ...
        # 添加每个目标的指标
        for i, tgt in enumerate(info["targets"]):
            log_data.update({
            ...
            if self.current_step % 100 == 0:
                log_data.update({
                    f"power_comm_{i}": tgt["comm"]["power"],
                    f"power_radar_{i}": tgt["radar"]["power"],
                    f"bw_comm_{i}": tgt["comm"]["bandwidth"]/1e6,
                    f"bw_radar_{i}": tgt["radar"]["bandwidth"]/1e6
                })
    ...
    def render(self, mode='human'):
        ...
        # 记录到WandB
        if mode != 'human' and self.wandb_enabled and self.current_step % 50 == 0:
            wandb.log({"env_status": wandb.Image(plt)}, commit=False)
    ...
    def check(self) -> Tuple[bool, dict]:
        """系统自检"""
        # 检查子模块
        comm_check = all([c.check()["status"] == "OK" for c in self.comm])
        radar_check = all([r.check()["status"] == "OK" for r in self.radar])
        ...
        report = {
            "modules": {
                "sat_channel": "OK" if comm_check else "ERROR",
                "radar_echo": "OK" if radar_check else "ERROR"
            },
            "action_space": {
                "valid": action_valid,
                "low": [0.1, 0.1],
        ...
        is_ok = all([comm_check, radar_check, action_valid])
